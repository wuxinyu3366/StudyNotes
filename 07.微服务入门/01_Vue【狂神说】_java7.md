...
MVVM框架已经把最脏最累的一块做好了， 我们开发者只需要处理和维护View Model， 更新数据视图就会自动得到相应更新，真正实现`事件驱动编程`。   View层展现的不是`Model`层的数据， 而是==`ViewModel`==的数据， 由`ViewModel`负责与`Model`层交互， 这就**完全解耦了View层和Model层， 这个解耦是至关重要的， 它是前后端分离方案实施的重要一环。**
...
![image-20210210205407531](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210210205407.png)
...
- 你看到的`v-bind`等被称为**指令**。指令带有**前缀v**以表示它们是Vue提供的特殊特性。可能你已经猜到了， 它们会在渲染的DOM上应用特殊的响应式行为在这里，该指令的意思是：“**将这个元素节点的title特性和Vue实例的message属性保持一致**”。
...
- 组件是可复用的`Vue`实例， 说白了就是一组可以**重复使用的模板**， 跟`JSTL`的**自定义标签**、`Thymeleal`的`th:fragment`等框架有着异曲同工之妙，通常一个应用会以一棵嵌套的组件树的形式来组织：
...
![image-20210219163520621](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210219163520.png)
...
![image-20210219202210195](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210219202210.png)
...
  服务器端的NodeJS遵循CommonsJS规范，该规范核心思想是允许模块通过**require方法**来同步加载所需依赖的其它模块，然后通过exports或module.exports来导出需要**暴露的接口**。
...
![image-20210224153427144](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224153427.png)
...
![image-20210224221733130](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224221740.png)
...
![image-20210224223401415](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224223401.png)
...
1.在`index.js`修改路由配置 , 主要增加了 props: true 属性
...
3. 接收参数为目标组件增加 props 属性
   ...
   **Profile.vue**
   ...
   ![image-20210224223734324](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224223734.png)
...
- > 说明：这里定义了两个路径，一个是 /main ，一个是 /goHome其中 /goHome 重定向到了 /main 路径，由此可以看出重定向不需要定义组件；
...
- 使用的话，只需要设置对应路径即可；
  ...
  **Main.vue**
  ...
  ![image-20210224224821047](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224224821.png)
...
![image-20210224232547690](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224232547.png)
...
![image-20210224233256281](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224233427.png)
...
4. 运行项目`npm run dev`看是否正常
   ...
   ![1595383163158](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224235021.png)
...
![image-20210224235421888](https://typora-wenjiuzhou.oss-cn-beijing.aliyuncs.com/20210224235422.png)
